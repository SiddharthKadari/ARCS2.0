...
DO NOT USE webcamtests.com
USE IT TO OPEN CAMERAS IF THE CAMERA FEED IS BLANK FOR TOO LONG, OTHERWISE DO NOT KEEP IT OPEN WHILE RUNNING SYSTEM
...
function b1Scan(col){
  if(col[2] > 150 && col[0] < 5 && col[1] < 100){
    return 'b';
  }else if(col[1] > 180 && col[0] < 130){
    return 'g';
  }else if(col[0] > 200){
    if(col[0] + col[1] > 400 && col[2] < 150 && col[1] > 230){
      return 'y';
    }else if(col[1] + col[2] + col[0] > 600){
      return 'w';
    }else if(col[1] + col[2] < 180){
      return 'r';
    }else{
      return 'o';
    }
...
function b2Scan(col){
  if(col[2] > 150 && col[0] < 5 && col[1] < 100){
    ...
    if(col[1] + col[2] + col[0] > 600){
      return 'w';
    }else if(col[0] + col[1] > 400 && col[2] < 150 && col[1] > 230){
      return 'y';
    }else if(col[1] + col[2] < 200){
      return 'r';
    }else{
      return 'o';
    }
...
function scanBlue(results){
  let state = '';
  state += b1Scan(results[8]);
  state += b1Scan(results[9]);
  state += b1Scan(results[10]) + '\n';
  state += b2Scan(results[11]);
  state += 'b';
  state += b1Scan(results[12]) + '\n';
  state += b1Scan(results[13]);
  state += b1Scan(results[14]);
  state += b1Scan(results[15]);
...
/*
parameter: order:
the order in which video feeds should be copied to the imgsCanvas
if the live feeds are in the order

feed1 :: feed2 :: feed3 :: feed4

then calling capture(2, 3, 1, 4)

will copy them down onto the canvas as

feed2 :: feed3 :: feed1 :: feed4

Blue should be on the left, and from there, adjacent colors should match
...
async function capture(...order){
  ...
  for(let img in camDomElems){
    subCapture(order[img], img); //copy down all the cameras' data simultaneously in different threads
...
async function subCapture(orderInd, img){
  imgsDomElem.getContext('2d').drawImage(camDomElems[orderInd - 1], 355*img, 0, 355, 200);
  finishedCaptures++;
}
...
async function initCams(){
  ...
  for(let i = 0; i < NUM_CAMS; i++){
    ...
    totalWidth += camDomElems[i].width;
  }
  imgsDomElem.width = totalWidth;
  imgsDomElem.height = HEIGHT;


}

//After swapping, sometimes it takes a while for the camera stream to properly reassign. After calling a swap, if one of the video streams is white or blank, just wait for up to a few minutes (longer than 5 minutes, then you should empty cache and hard reload the localhost)
function swap(s1, s2){
...
async function handleMessage(msg){
  console.log(performance.now() - start - 200);
  console.log(msg);
}
...
//   await initCams();
  ...
  //   a = performance.now();
  //   let temp = countQuarters(min2phase.solve(min2phase.randomCube()));
  //   c[0] += temp[0];
  //   c[1] += temp[1];
  //   c[2] += temp[2];
  //   c[3] += temp[3];
  //   t += performance.now() - a;
  //   if(i % (epochs/100) == 0 && i != 0)
  //     console.log(i / (epochs/100) + '%');
  // }
  // c[0] = c[0]/epochs;
  // c[1] = c[1]/epochs;
  // c[2] = c[2]/epochs;
  // c[3] = c[3]/epochs;
  // console.log(c);
  // console.log(t/epochs);

  // for(let i = 0; i < 100; i++){
  // a = performance.now();
  //
  // console.log(countQuarters(min2phase.solve(min2phase.randomCube())));
  //
...
function simplifyAlg(alg){
  ...
  return simplifiedAlg + alg[alg.length - 1];
}
...
function countQuarters(alg){
  ...
  alg = alg.replaceAll('f', 'e');
  alg = alg.replaceAll('r', 'q');
  alg = alg.replaceAll('l', 'k');

  alg = alg.replaceAll('c', 'd');
  alg = alg.replaceAll('o', 'p');
  alg = alg.replaceAll('i', 'j');

  alg = alg.replaceAll('a', 'b');
  alg = alg.replaceAll('m', 'n');
  alg = alg.replaceAll('g', 'h');

  for(let i = 0; i < alg.length; i++){
    c++;
    if(i < alg.length - 1 && (alg[i+1].charCodeAt(0) - alg[i].charCodeAt(0)) % 6 == 0){
      if(alg[i+1].charCodeAt(0) > 108 || alg[i] > 108){
        c++;
        halfTurnC++;
      }else {
        quarterTurnC++;
      ...
      i++;
    }else if(alg[i].charCodeAt(0) > 108){
      c++;
      halfTurnC++;
    }else {
      quarterTurnC++;
...
//Total quarters, halfs, quarters,    run time estimate
  return [c, halfTurnC, quarterTurnC, halfTurnC * 45 + quarterTurnC * 26];
}
...
async function fullTest(){
  ...
  let alg = await time(min2phase.solve, min2phase.randomCube(), 20);
  console.log(alg);

  console.log("###SEND TEST###");
  a = performance.now();
  console.log(alg);
  await sendMessage("ABCDEFGHIJKLMNOPQRST");
